...
https://github.com/test502git/awvs13_batch_py3.git	github脚本

https://192.168.2.238:13443/Acunetix-API-Documentation.html#acunetix-scanner-api	awvs13 api官方文档

https://www.sqlsec.com/2020/04/awvsapi.html	国光大佬的学习blog

**path：**

avws_url:https://192.168.2.238:13443/

awvs_key:1986ad8c0a5b3df4d7028d5f3c06e936c256b8ed8af104ea2b4851702980f49ef
...
```http
https://192.168.2.238:13443/api/v1/me
```

curl

```bash
[root@localhost ~]# curl -X GET https://192.168.2.238:13443/api/v1/me \
>   -H 'Accept: application/json' \
>   -H 'X-Auth: 1986ad8c0a5b3df4d7028d5f3c06e936c256b8ed8af104ea2b4851702980f49ef' \
>   -k
{
 "child_account": false,
...
api_url = 'https://192.168.2.238:13443/api/v1/me'
headers = {
    'X-Auth': '1986ad8c0a5b3df4d7028d5f3c06e936c256b8ed8af104ea2b4851702980f49ef',
...
```bash
[root@localhost code]# python3 awvs.py 
{'child_account': False, 'email': 'admin@admin.com', 'enabled': True, 'first_name': 'Administrator', 'lang': 'cn', 'user_id': '986ad8c0-a5b3-df4d-7028-d5f3c06e936c', 'totp_enabled': False, 'su': True}
```

其他api可参考awvs13 api官方文档
...
import requests
import json,ast
from requests.packages.urllib3.exceptions import InsecureRequestWarning#关闭SSL证书验证
import configparser#解析ini配置文件
cf = configparser.ConfigParser()
requests.packages.urllib3.disable_warnings(InsecureRequestWarning)#关闭安全请求警告
cf.read(r"awvs_config.ini",encoding='utf-8')
secs=cf.sections()
awvs_url =cf.get('awvs_url_key','awvs_url')
apikey = cf.get('awvs_url_key','api_key')
headers = {'Content-Type': 'application/json',"X-Auth": apikey}#设置请求头
add_count_suss=0
error_count=0
target_scan=False
target_list=[]
...
```http
api:https://192.168.2.238:13443/Acunetix-API-Documentation.html#get_targets
```

```python
def get_target_list():
    global pages,target_list#引入全局变量
    while 1:
        target_dict={}
        ...
        result = json.loads(r.content.decode())#json格式化处理
        #{'targets': [], 'pagination': {'count': 1, 'cursor_hash': '8d5513d7236f5b13c8abc7410eabe6a4', 'cursors': ['10'], 'sort': None}}
        try:#异常处理
...
```http
api:https://192.168.2.238:13443/Acunetix-API-Documentation.html#add_targets
```
...
def addTask(url,target):
    try:
        ...
        data = {"targets":[{"address": target,"description":"此url为脚本添加"}],"groups":[]}#{'targets': [{'address': 'https://broker.mqtt.pandora.xiaomi.com', 'description': '此url为脚本添加'}], 'groups': []}
        r = requests.post(url, headers=headers, data=json.dumps(data), timeout=30, verify=False)
        result = json.loads(r.content.decode())#{'targets': [{'address': 'https://broker.mqtt.pandora.xiaomi.com', 'criticality': 10, 'description': '此url为脚本添加', 'type': 'default', 'domain': 'broker.mqtt.pandora.xiaomi.com', 'target_id': '3a611415-b71a-41c9-ab61-0aea6f347218', 'target_type': None, 'canonical_address': 'broker.mqtt.pandora.xiaomi.com', 'canonical_address_hash': 'f945612ecd69696cc09a30b500741d6f'}]}
...
```http
api:https://192.168.2.238:13443/Acunetix-API-Documentation.html#schedule_scan
```
...
```http
api:https://192.168.2.238:13443/Acunetix-API-Documentation.html#get_workers_assigned_to_target
```
    ...
    while 1:
        ...
        try:
            ...
            for targetsid in range(len(result['targets'])):
                ...
                targets_address = result['targets'][targetsid]['address']#获取目标详情
                #print(targets_id,targets_address)
                try:
                    del_log=requests.delete(awvs_url+'/api/v1/targets/'+targets_id,headers=headers, timeout=30, verify=False)#删除目标
...
```http
api:https://192.168.2.238:13443/Acunetix-API-Documentation.html#get_scanning_profiles
```
